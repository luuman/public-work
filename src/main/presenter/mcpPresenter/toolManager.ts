...
import { MCP_EVENTS, NOTIFICATION_EVENTS } from '@/events/events';
import {
  MCPToolCall,
  MCPToolDefinition,
  MCPToolResponse,
...
export class ToolManager {
  ...
  private toolNameToTargetMap: Map<
    string,
  ...
  private handleServerListUpdate = (): void => {
    console.info(
      'MCP client list updated, clearing tool definitions cache and target map.',
    );
  ...
  public async getAllToolDefinitions(
        ...
        for (const tool of clientTools) {
          if (toolNameToServerMap.has(tool.name)) {
            ...
            if (originalServerName !== client.serverName) {
              ...
              const originalServerRenames =
                toolsToRename.get(originalServerName) || new Set();
    ...
    for (const client of clients) {
      try {
        ...
        for (const tool of clientTools) {
          ...
          if (this.toolNameToTargetMap) {
            this.toolNameToTargetMap.set(finalName, {
              client: client,
    ...
    console.info(
      `Cached ${results.length} final tool definitions and populated target map.`,
    );
  ...
  private checkToolPermission(
    ...
    if (autoApprove.includes('all')) {
      console.log(
        `[ToolManager] Permission granted: server '${serverName}' has 'all' permissions`,
      );
      return true;
    }
    ...
    console.log(
      `[ToolManager] Tool '${originalToolName}' requires '${permissionType}' permission`,
    );
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.error(
            'Error parsing tool call arguments even after jsonrepair:',
            argsString,
            e,
          );
      ...
      if (!serverConfig) {
        console.error(
          `Configuration for server '${toolServerName}' not found.`,
        );
        return {
          toolCallId: toolCall.id,
  ...
  async grantPermission(
    ...
    if (remember) {
      ...
      console.log(
        `[ToolManager] Temporary permission granted (session-scoped)`,
      );
    }
  }
  ...
  private async updateServerPermissions(
    ...
    try {
      console.log(
        `[ToolManager] Updating server ${serverName} permissions: ${permissionType}`,
      );
      const servers = await this.configPresenter.getMcpServers();
      ...
      if (serverConfig) {
        ...
        if (permissionType === 'all') {
          autoApprove = autoApprove.filter(
            (p) => p !== 'read' && p !== 'write',
          );
        ...
        console.log(
          `[ToolManager] After update - Server ${serverName} permissions:`,
          autoApprove,
        );
        ...
        console.error(
          `[ToolManager] Server configuration not found for: ${serverName}`,
        );
      }
    } catch (error) {
      console.error(
        '[ToolManager] Failed to update server permissions:',
        error,
      );
    }
  }
