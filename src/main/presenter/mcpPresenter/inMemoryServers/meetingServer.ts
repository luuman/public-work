...
const ParticipantSchema = z
  ...
  .refine(
    (data) => {
      ...
      const hasTitle =
        data.tab_title !== undefined && data.tab_title.trim() !== '';
...
export class MeetingServer {
  ...
  private setupRequestHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        return {
          content: [
            { type: 'text', text: '会议已成功启动，正在后台进行中...' },
          ],
        };
      } catch (error: any) {
        return {
   ...
   * @param args 会议参数
   */
  private async organizeMeeting(
    args: z.infer<typeof StartMeetingArgsSchema>,
  ): Promise<void> {
    ...
    const allChatTabs =
      await presenter.tabPresenter.getWindowTabsData(mainWindowId);
    ...
    for (const p of participants) {
      ...
      if (!foundByTabTitle) {
        ...
        let conversationId =
          await presenter.threadPresenter.getActiveConversationId(tabData.id);
        ...
        if (!conversationId) {
          console.log(
            `参会者 (ID: ${tabData.id}) 没有活动会话，正在为其创建...`,
          );
          ...
          try {
            ...
            console.log(
              `会话 ${conversationId} 在Tab ${tabData.id} 中已成功激活。`,
            );
          } catch (error) {
        ...
        meetingParticipants.push({
          ...
          profile:
            p.profile || `你可以就“${topic}”这个话题，自由发表你的看法和观点。`,
        });
      } else {
    ...
    const participantNames = meetingParticipants
      .map((p) => p.meetingName)
      .join('、');
...
会议现在开始。请等待你的发言回合。
`;
      eventBus.sendToTab(p.tabId, MEETING_EVENTS.INSTRUCTION, {
        prompt: initPrompt,
      });
    ...
    for (let round = 1; round <= rounds; round++) {
      for (const speaker of meetingParticipants) {
        ...
        eventBus.sendToTab(speaker.tabId, MEETING_EVENTS.INSTRUCTION, {
          prompt: speakPrompt,
        });

        // 等待并捕获真正的、需要被记录和转发的发言内容
        const speechMessage = await this.waitForResponse(
          speaker.conversationId,
        );
        const speechText = Array.isArray(speechMessage.content)
          ? speechMessage.content.find((c: any) => c.type === 'content')
              ?.content || '[无内容]'
