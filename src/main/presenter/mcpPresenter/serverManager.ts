...
export class ServerManager {
  ...
  loadRegistryFromCache(): void {
    ...
    if (effectiveRegistry) {
      ...
      console.log(
        `[NPM Registry] Loaded effective registry: ${effectiveRegistry}`,
      );
    } else {
      this.npmRegistry = null;
      this.uvRegistry = null;
      console.log(
        '[NPM Registry] No effective registry, will use default or detect',
      );
    }
  }
  ...
  async testNpmRegistrySpeed(useCache: boolean = true): Promise<string> {
    ...
    if (successfulResults.length === 0) {
      ...
      console.log(
        `[NPM Registry] Best registry: ${bestRegistry} (${successfulResults[0].time}ms)`,
      );
    }
  ...
  async updateNpmRegistryInBackground(): Promise<void> {
    try {
      ...
      if (this.configPresenter.isNpmRegistryCacheValid?.()) {
        console.log(
          '[NPM Registry] Cache is still valid, skipping background update',
        );
        return;
      }
