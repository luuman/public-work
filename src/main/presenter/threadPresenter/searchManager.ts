...
function generateExtractorScript(selectorConfig: SelectorConfig): string {
  return EXTRACTOR_SCRIPT_TEMPLATE.replace(
    '{{ITEMS_SELECTOR}}',
    selectorConfig.itemsSelector,
  )
...
function generateDefaultEngines() {
  return [
    ...
    },
    {
      ...
      extractorScript: generateExtractorScript(
        searchEngineSelectors['google-scholar'],
      ),
    },
    {
      id: 'baidu-xueshu',
      ...
      extractorScript: generateExtractorScript(
        searchEngineSelectors['baidu-xueshu'],
      ),
    },
    {
      id: 'duckduckgo',
...
export class SearchManager {
   ...
   */
  private async fetchSelectorsFromCloud(): Promise<Record<
    string,
   ...
   * 对一个完整的选择器配置进行安全处理
   */
  private sanitizeSelectorConfig(
  ...
  private regenerateDefaultEngines(): SearchEngineTemplate[] {
    return [
      ...
      },
      {
        ...
        searchUrl: 'https://scholar.google.com/scholar?q={query}',
        extractorScript: generateExtractorScript(
          this.currentSelectors['google-scholar'],
        ),
      },
      {
        id: 'baidu-xueshu',
        ...
        searchUrl: 'https://xueshu.baidu.com/s?wd={query}',
        extractorScript: generateExtractorScript(
          this.currentSelectors['baidu-xueshu'],
        ),
      },
      {
        id: 'duckduckgo',
  ...
  private async initSearchWindow(
   ...
   * 使用AI模型分析页面内容提取搜索结果
   */
  private async fallbackExtractSearchResults(
    window: BrowserWindow,
  ): Promise<SearchResult[]> {
    try {
