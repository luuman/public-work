...
import { approximateTokenSize } from '';
...
export class ThreadPresenter implements IThreadPresenter {
  ...
  constructor(
    ...
    eventBus.on(TAB_EVENTS.CLOSED, (tabId: number) => {
      if (this.activeConversationIds.has(tabId)) {
        ...
        console.log(
          `ThreadPresenter: Cleaned up conversation binding for closed tab ${tabId}.`,
        );
      }
    });
  ...
  async handleLLMAgentEnd(msg: LLMAgentEventData) {
    ...
    if (state) {
      ...
      const hasPendingPermissions = state.message.content.some(
        (block) =>
          block.type === 'tool_call_permission' && block.status === 'pending',
      );
      ...
      console.log(
        `[ThreadPresenter] Finalizing message ${eventId} - no pending permissions`,
      );
  ...
  private async finalizeMessage(
    ...
    const generationTime =
      Date.now() - (state.firstTokenTime ?? state.startTime);
  ...
  // 处理会话更新和标题生成
  private async handleConversationUpdates(
    state: GeneratingMessageState,
  ): Promise<void> {
    const conversation = await this.sqlitePresenter.getConversation(
  ...
  async handleLLMAgentResponse(msg: LLMAgentEventData) {
    ...
    if (state) {
      ...
      const finalizeLastBlock = () => {
        ...
        if (lastBlock) {
          if (
            lastBlock.type === 'tool_call_permission' &&
          ...
          if (
            ...
            if (searchResults.length > 0) {
              ...
              const existingSearchBlock =
                state.message.content.length > 0 &&
                state.message.content[0].type === 'search'
              ...
              if (existingSearchBlock) {
                ...
                if (existingSearchBlock.extra) {
                  ...
                  existingSearchBlock.extra.total =
                    (existingSearchBlock.extra.total || 0) +
        ...
        if (tool_call === 'start') {
          ...
          if (toolCallBlock && toolCallBlock.type === 'tool_call') {
            ...
            if (toolCallBlock.tool_call) {
              ...
              toolCallBlock.tool_call.server_description =
                tool_call_server_description;
            }
          }
          ...
          state.message.content.push({
            ...
            extra: {
              ...
              serverName:
                permission_request?.serverName || tool_call_server_name || '',
            ...
            if (tool_call === 'error') {
              ...
              if (toolCallBlock.tool_call) {
                if (typeof tool_call_response === 'string') {
                  toolCallBlock.tool_call.response =
                    tool_call_response || '执行失败';
                } else {
                  toolCallBlock.tool_call.response =
                    JSON.stringify(tool_call_response);
                }
              }
            } else {
  ...
  async createConversation(
    ...
    if (!options.forceNewAndActivate) {
      if (latestConversation) {
        const { list: messages } = await this.getMessages(
  ...
  async updateConversationSettings(
    ...
    if (
      settings.modelId &&
      settings.modelId !== conversation.settings.modelId
    ) {
      // 获取模型配置
  ...
  async getMessages(
    ...
    return await this.messageManager.getMessageThread(
      conversationId,
      page,
      pageSize,
    );
  ...
  async getContextMessages(conversationId: string): Promise<Message[]> {
    ...
    const messages = await this.messageManager.getContextMessages(
      conversationId,
      messageCount,
    );
    ...
    return messages.map((msg) => {
      if (msg.role === 'user') {
        ...
        if (msgContent.content) {
          (newMsg.content as UserMessageContent).text =
  ...
  private async generateAIResponse(
    ...
    try {
      const triggerMessage =
        await this.messageManager.getMessage(userMessageId);
      if (!triggerMessage) {
  ...
  private async getMessageHistory(
    ...
    return messages.slice(
      Math.max(0, targetIndex - limit + 1),
      targetIndex + 1,
    );
  }
  ...
  async getSearchResults(messageId: string): Promise<SearchResult[]> {
    const results = await this.sqlitePresenter.getMessageAttachments(
      messageId,
      'search_result',
    );
    return (
      results.map((result) => JSON.parse(result.content) as SearchResult) ?? []
    );
  }
  ...
  async startStreamCompletion(conversationId: string, queryMsgId?: string) {
    ...
    try {
      ...
      // 1. 获取上下文信息
      const { conversation, userMessage, contextMessages } =
        await this.prepareConversationContext(conversationId, queryMsgId);
      ...
      const { userContent, urlResults, imageFiles } =
        await this.processUserMessageContent(userMessage as UserMessage);
      ...
      await this.messageManager.handleMessageError(
        state.message.id,
        String(error),
      );
      throw error;
  ...
  async continueStreamCompletion(conversationId: string, queryMsgId: string) {
    ...
    try {
      ...
      const { conversation, contextMessages, userMessage } =
        await this.prepareConversationContext(conversationId, state.message.id);
  ...
  // 查找特定会话的生成状态
  private findGeneratingState(
    conversationId: string,
  ): GeneratingMessageState | null {
    return (
  ...
  private async prepareConversationContext(
    ...
    // 任何情况都使用最新配置
    const webSearchEnabled = this.configPresenter.getSetting(
      'input_webSearch',
    ) as boolean;
    const thinkEnabled = this.configPresenter.getSetting(
      'input_deepThinking',
    ) as boolean;
  ...
  private async processUserMessageContent(userMessage: UserMessage): Promise<{
    ...
    // 从用户消息中提取并丰富URL内容
    const urlResults = await ContentEnricher.extractAndEnrichUrls(
      userMessage.content.text,
    );
  ...
  private async preparePromptContent(
    ...
    const { systemPrompt, contextLength, artifacts, enabledMcpTools } =
      conversation.settings;
    ...
    // 计算token数量
    const searchPromptTokens = searchPrompt
      ? approximateTokenSize(searchPrompt ?? '')
      : 0;
    const systemPromptTokens = systemPrompt
      ? approximateTokenSize(systemPrompt ?? '')
      : 0;
    const userMessageTokens = approximateTokenSize(
      userContent + enrichedUserMessage,
    );
    const mcpTools =
  ...
  private selectContextMessages(
    ...
    for (const msg of messages) {
      ...
      const msgContent =
        msg.role === 'user' ? (msg.content as UserMessageContent) : null;
      ...
      if (currentLength + msgTokens <= remainingContextLength) {
        ...
        if (msg.role === 'user') {
          ...
          if (userMsgContent.content && !userMsgContent.text) {
            userMsgContent.text = this.formatUserMessageContent(
  ...
  private mergeConsecutiveMessages(messages: ChatMessage[]): ChatMessage[] {
    ...
    for (let i = 1; i < messages.length; i++) {
      ...
      if (allowMessagePropertiesMerge) {
        ...
        let newCombinedContent: string | ChatMessageContent[] | undefined =
          undefined;
        ...
        if (LMC === undefined && CMC === undefined) {
          ...
          typeof LMC === 'string' &&
          (typeof CMC === 'string' || CMC === undefined)
        ) {
          ...
          Array.isArray(LMC) &&
          (Array.isArray(CMC) || CMC === undefined)
        ) {
  ...
  async retryMessage(messageId: string): Promise<AssistantMessage> {
    ...
    const userMessage = await this.messageManager.getMessage(
      message.parentId || '',
    );
    if (!userMessage) {
  ...
  async stopConversationGeneration(conversationId: string): Promise<void> {
    ...
    await Promise.all(
      messageIds.map((messageId) => this.stopMessageGeneration(messageId)),
    );
  }
  ...
  async summaryTitles(
    ...
    const targetConversationId =
      conversationId ??
      (tabId !== undefined ? this.activeConversationIds.get(tabId) : undefined);
    ...
    summaryProviderId =
      this.searchAssistantProviderId || conversation.settings.providerId;
  ...
  async getMessageExtraInfo(
    messageId: string,
    type: string,
  ): Promise<Record<string, unknown>[]> {
    const attachments = await this.sqlitePresenter.getMessageAttachments(
      messageId,
      type,
    );
  ...
  async getMainMessageByParentId(
    ...
    const message = await this.messageManager.getMainMessageByParentId(
      conversationId,
      parentId,
    );
    if (!message) {
  ...
  async forkConversation(
    ...
    try {
      ...
      const sourceConversation =
        await this.sqlitePresenter.getConversation(targetConversationId);
      ...
      const newConversationId =
        await this.sqlitePresenter.createConversation(newTitle);
      ...
      for (const msg of messageHistory) {
        ...
        // 获取消息序号
        const orderSeq =
          (await this.sqlitePresenter.getMaxOrderSeq(newConversationId)) + 1;
  ...
  async translateText(text: string, tabId: number): Promise<string> {
    try {
      ...
      if (!conversation) {
        ...
        const models = await this.llmProviderPresenter.getModelList(
          defaultProvider.id,
        );
  ...
  private async broadcastThreadListUpdate(): Promise<void> {
    // 1. 获取所有会话 (假设9999足够大)
    const result = await this.sqlitePresenter.getConversationList(
      1,
      this.fetchThreadLength,
    );
    ...
    const finalGroupedList = Array.from(groupedThreads.entries()).map(
      ([dt, dtThreads]) => ({
        dt,
        dtThreads,
      }),
  ...
  private exportToHtml(
    ...
    lines.push(
      '      .user-message { border-left-color: #3b82f6; background: #1e293b; }',
    );
    lines.push(
      '      .assistant-message { border-left-color: #10b981; background: #064e3b; }',
    );
    lines.push(
      '      .tool-call { background: #1f2937; border-color: #374151; }',
    );
    lines.push(
      '      .search-block { background: #1e3a8a; border-color: #1d4ed8; }',
    );
    lines.push(
      '      .error-block { background: #7f1d1d; border-color: #dc2626; }',
    );
    lines.push(
      '      .reasoning-block { background: #581c87; border-color: #7c3aed; }',
    );
    lines.push(
      '      .code { background: #1f2937; border-color: #374151; color: #f3f4f6; }',
    );
    lines.push(
      '      .attachments { background: #78350f; border-color: #d97706; }',
    );
    lines.push('    }');
    lines.push(
    ...
    lines.push(
      '    .header p { margin: 4px 0; font-size: 0.875rem; color: #6b7280; }',
    );
    lines.push(
      '    .message { margin-bottom: 32px; border-radius: 12px; padding: 20px; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1); }',
    );
    lines.push(
      '    .user-message { background: #f8fafc; border-left: 4px solid #3b82f6; }',
    );
    lines.push(
    ...
    lines.push(
      '    .message-time { font-size: 0.75rem; color: #9ca3af; font-weight: 400; }',
    );
    lines.push(
    ...
    for (const message of messages) {
      ...
      if (message.role === 'user') {
        ...
        lines.push(
          `    <div>${this.escapeHtml(messageText).replace(/\n/g, '<br>')}</div>`,
        );
          ...
          switch (block.type) {
            ...
            case 'reasoning_content':
              if (block.content) {
                ...
                lines.push(
                  `      <div class="code">${this.escapeHtml(block.content)}</div>`,
                );
                lines.push('    </div>');
              }
              break;
            ...
            case 'error':
              if (block.content) {
                ...
                lines.push(
                  `      <p><code>${this.escapeHtml(block.content)}</code></p>`,
                );
                lines.push('    </div>');
              }
              break;
  ...
  async handlePermissionResponse(
    ...
    try {
      ...
      console.log(
        `[ThreadPresenter] Updated permission block status to: ${permissionBlock.status}`,
      );
      ...
      if (granted) {
        ...
        try {
          ...
          console.error(
            `[ThreadPresenter] Failed to grant permission:`,
            permissionError,
          );
          // 权限授予失败，将状态更新为错误
      ...
      console.error(
        `[ThreadPresenter] Failed to handle permission response:`,
        error,
      );
      ...
      try {
        ...
        console.error(
          `[ThreadPresenter] Failed to update message error status:`,
          updateError,
        );
      }
  ...
  private async restartAgentLoopAfterPermission(
    ...
    try {
      ...
      console.log(
        `[ThreadPresenter] Found message in conversation: ${conversationId}`,
      );
      ...
      const permissionBlock = content.find(
        (block) =>
          block.type === 'tool_call_permission' && block.status === 'granted',
      );
      ...
      if (permissionBlock?.extra?.serverName) {
        ...
        try {
          ...
          const serverConfig =
            servers[permissionBlock.extra.serverName as string];
          console.log(
          ...
          console.warn(
            `[ThreadPresenter] Failed to verify server permissions:`,
            configError,
          );
        }
      }
      ...
      if (state) {
        console.log(
          `[ThreadPresenter] Message still in generating state, resuming from memory`,
        );
      ...
      // 否则重新启动完整的agent loop
      console.log(
        `[ThreadPresenter] Message not in generating state, starting fresh agent loop`,
      );
      ...
      console.log(
        `[ThreadPresenter] Created new generating state for message: ${messageId}`,
      );
  ...
  // 权限被拒绝后完成消息
  private async finalizeMessageAfterPermissionDenied(
    messageId: string,
  ): Promise<void> {
    console.log(
      `[ThreadPresenter] Finalizing message after permission denied: ${messageId}`,
    );
    ...
    try {
      ...
      console.log(
        `[ThreadPresenter] Message finalized after permission denial: ${messageId}`,
      );
    } catch (error) {
      console.error(
        `[ThreadPresenter] Failed to finalize message after permission denial:`,
        error,
      );
    }
  }
  ...
  private async resumeStreamCompletion(
    ...
    try {
      console.log(
        `[ThreadPresenter] Resuming stream completion for message: ${messageId}`,
      );
      ...
      // 查找被权限中断的工具调用
      const pendingToolCall = this.findPendingToolCallAfterPermission(
      ...
      const { contextMessages, userMessage } =
        await this.prepareConversationContext(
          conversationId,
      ...
      console.log(
        `[ThreadPresenter] Built continue context for tool: ${pendingToolCall.name}`,
      );
      ...
      console.error(
        '[ThreadPresenter] Failed to resume stream completion:',
        error,
      );
  ...
  private async waitForMcpServiceReady(
    ...
    console.log(
      `[ThreadPresenter] Waiting for MCP service ${serverName} to be ready...`,
    );
    ...
    return new Promise((resolve) => {
      const checkReady = async () => {
        try {
          // 检查服务是否正在运行
          const isRunning =
            await presenter.mcpPresenter.isServerRunning(serverName);

          if (isRunning) {
            // 服务正在运行，再等待一下确保完全初始化
            setTimeout(() => {
              console.log(
                `[ThreadPresenter] MCP service ${serverName} is ready`,
              );
              resolve();
            }, 200);
            return;
          ...
          console.error(
            `[ThreadPresenter] Error checking MCP service status:`,
            error,
          );
          resolve(); // 出错也继续，避免阻塞
        }
