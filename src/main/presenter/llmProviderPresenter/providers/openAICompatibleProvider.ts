...
export class OpenAICompatibleProvider extends BaseLLMProvider {
  ...
  constructor(provider: LLM_PROVIDER, configPresenter: ConfigPresenter) {
    ...
    if (
      OpenAICompatibleProvider.NO_MODELS_API_LIST.includes(
        this.provider.id.toLowerCase(),
      )
    ) {
  ...
  protected createOpenAIClient(): void {
    ...
    if (this.provider.id === 'azure-openai') {
      try {
        const apiVersion =
          this.configPresenter.getSetting<string>('azureApiVersion');
  ...
  protected formatMessages(
    ...
    return messages.map((msg) => {
      ...
      if (msg.role === 'assistant' && msg.tool_calls) {
        (baseMessage as ChatCompletionAssistantMessageParam).tool_calls =
          msg.tool_calls;
      }
      if (msg.role === 'tool') {
        (baseMessage as ChatCompletionToolMessageParam).tool_call_id =
          msg.tool_call_id || '';
      }
  ...
  private async *handleImgGeneration(
    ...
    if (!prompt) {
      console.error(
        '[handleImgGeneration] Could not extract prompt for image generation.',
      );
      yield {
        type: 'error',
        error_message: 'Could not extract prompt for image generation.',
      };
    ...
    try {
      ...
      if (imageUrls.length > 0) {
        ...
        try {
          ...
          console.error(
            '[handleImgGeneration] Failed to delete temporary file:',
            e,
          );
        }
      } else {
      ...
      if (result.data && (result.data[0]?.url || result.data[0]?.b64_json)) {
        ...
        try {
          ...
          yield {
            ...
            image_data: {
              ...
              mimeType: result.data[0]?.url
                ? 'deepchat/image-url'
                : 'deepchat/image-base64',
            },
          };
          yield { type: 'stop', stop_reason: 'complete' };
        }
        ...
        console.error(
          '[handleImgGeneration] No image data received from API.',
          result,
        );
        yield {
          type: 'error',
          error_message: 'No image data received from API.',
        };
        yield { type: 'stop', stop_reason: 'error' };
      }
    } catch (error: unknown) {
      ...
      console.error(
        '[handleImgGeneration] Error during image generation:',
        errorMessage,
      );
      yield {
        type: 'error',
  ...
  private async *handleChatCompletion(
    ...
    const apiTools =
      tools.length > 0 && supportsFunctionCall
        ? await presenter.mcpPresenter.mcpToolsToOpenAITools(
            tools,
            this.provider.id,
          )
        : undefined;
    ...
    if (apiTools && apiTools.length > 0 && supportsFunctionCall)
    ...
    for await (const chunk of stream) {
        ...
        for (const toolCallDelta of delta.tool_calls) {
          ...
          if (currentToolCallId) {
            if (!nativeToolCalls[currentToolCallId]) {
              nativeToolCalls[currentToolCallId] = {
                name: '',
                arguments: '',
                completed: false,
              };
      ...
      if (choice?.finish_reason) {
        ...
        if (reasonFromAPI === 'tool_calls') {
          ...
          console.warn(
            `[handleChatCompletion] Unhandled finish reason: ${reasonFromAPI}`,
          );
          stopReason = 'error';
        }
        /*
        ...
        while (pendingBuffer.length > 0) {
          if (inThinkBlock) {
            ...
            if (
              ...
              const textBeforeTag = pendingBuffer.slice(
                0,
  ...
  protected parseFunctionCalls(
    ...
    try {
          ...
          try {
            ...
            if (
              ...
              if (keys.length === 1) {
                ...
                if (
                  potentialToolCall &&
                  typeof potentialToolCall === 'object'
                ) {
                  if (
                    potentialToolCall.name &&
                    potentialToolCall.arguments !== undefined
                  ) {
      ...
      console.log(
        `[parseFunctionCalls] Returning ${toolCalls.length} parsed tool calls.`,
      ); // Log final count
      return toolCalls;
    } catch (error) {
      console.error(
  ...
  async getDimensions(modelId: string): Promise<LLM_EMBEDDING_ATTRS> {
    switch (modelId) {
      ...
      default:
        try {
          const embeddings = await this.getEmbeddings(modelId, [
            EMBEDDING_TEST_KEY,
          ]);
          return {
