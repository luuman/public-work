...
import { OLLAMA_EVENTS } from '@/events/events';
...
import { CONFIG_EVENTS, RATE_LIMIT_EVENTS } from '@/events/events';
...
export class LLMProviderPresenter implements ILlmProviderPresenter {
  ...
  // 速率限制相关属性
  private providerRateLimitStates: Map<string, ProviderRateLimitState> =
    new Map();
  ...
  private createProviderInstance(
    ...
    try {
      ...
      console.error(
        `Failed to create provider instance for ${provider.id}:`,
        error,
      );
      return undefined;
    }
  ...
  getCurrentProvider(): LLM_PROVIDER | null {
    return this.currentProviderId
      ? this.providers.get(this.currentProviderId) || null
      : null;
  }
  ...
  setProviders(providers: LLM_PROVIDER[]): void {
    ...
    if (
      ...
      !providers.find((p) => p.id === this.currentProviderId)
    ) {
  ...
  async getModelList(providerId: string): Promise<MODEL_META[]> {
    ...
    models = models.map((model) => {
      ...
      if (config.isUserDefined) {
        ...
        model.vision =
          model.vision !== undefined ? model.vision : config.vision;
        model.functionCall =
          model.functionCall !== undefined
            ? model.functionCall
            : config.functionCall;
        model.reasoning =
          model.reasoning !== undefined ? model.reasoning : config.reasoning;
  ...
  updateProviderRateLimit(
    ...
    if (provider) {
      ...
      console.log(
        `[LLMProviderPresenter] Updated persistent config for ${providerId}`,
      );
    }
  }
  ...
  async *startStreamCompletion(
    ...
    console.log(
      `[Agent Loop] Starting agent loop for event: ${eventId} with model: ${modelId}`,
    );
    if (!this.canStartNewStream()) {
      // Instead of throwing, yield an error event
      yield {
        type: 'error',
        data: { eventId, error: '已达到最大并发流数量限制' },
      };
      return;
    ...
    try {
      ...
      while (needContinueConversation) {
        ...
        const currentToolChunks: Record<
          string,
          { name: string; arguments_chunk: string }
        > = {};

        try {
          console.log(
            `[Agent Loop] Iteration ${toolCallCount + 1} for event: ${eventId}`,
          );
          const mcpTools =
          ...
          for await (const chunk of stream) {
            ...
            switch (chunk.type) {
              ...
              case 'error':
                console.error(
                  `Provider stream error for event ${eventId}:`,
                  chunk.error_message,
                );
                yield {
                  type: 'error',
          ...
          if (
            currentContent ||
            (needContinueConversation && currentToolCalls.length > 0)
          ) {
          ...
          if (needContinueConversation && currentToolCalls.length > 0) {
            for (const toolCall of currentToolCalls) {
              ...
              if (toolCallCount >= MAX_TOOL_CALLS) {
                console.warn(
                  'Max tool calls reached during execution phase for event:',
                  eventId,
                );
                yield {
                  type: 'response',
              ...
              const toolDef = (
                await presenter.mcpPresenter.getAllToolDefinitions(
                  enabledMcpTools,
                )
              ...
              if (!toolDef) {
                console.error(
                  `Tool definition not found for ${toolCall.name}. Skipping execution.`,
                );
              ...
              try {
                  ...
                  yield {
                    ...
                    data: {
                      ...
                      tool_call_server_name:
                        toolResponse.rawData.permissionRequest?.serverName,
                      tool_call_server_icons: toolDef.server.icons,
                      tool_call_server_description: toolDef.server.description,
                      tool_call_response: toolResponse.content,
                      permission_request:
                        toolResponse.rawData.permissionRequest,
                    },
                  };
                ...
                if (supportsFunctionCall) {
                  ...
                  if (lastAssistantMessage) {
                    ...
                    lastAssistantMessage =
                      conversationMessages[conversationMessages.length - 1]; // Update lastAssistantMessage reference
                  }
                ...
                const supportsFunctionCallInAgent =
                ...
                if (supportsFunctionCallInAgent) {
                  ...
                  if (lastAssistantMessage) {
                    ...
                    conversationMessages.push({
                      role: 'assistant',
                      content: [
                        { type: 'text', text: formattedErrorText + '\n' },
                      ],
                    });
                  }
          ...
          if (abortController.signal.aborted) {
            console.log(
              `Agent loop aborted during inner try-catch for event ${eventId}`,
            );
      ...
      if (abortController.signal.aborted) {
        console.log(
          `Agent loop aborted during outer try-catch for event ${eventId}`,
        );
      } else {
  ...
  async generateCompletionStandalone(
    ...
    let response = '';
    try {
      const llmResponse = await provider.completions(
        messages,
        modelId,
        temperature,
        maxTokens,
    ...
    try {
      ...
      if (modelId) {
        try {
          ...
          return {
            isOk: false,
            errorMsg: `Model test failed: ${errorMessage}`,
          };
        }
      } else {
      ...
      return {
        isOk: false,
        errorMsg: `Provider check failed: ${errorMessage}`,
      };
    }
  }
  ...
  async refreshModels(providerId: string): Promise<void> {
    try {
      ...
      console.error(
        `Failed to refresh models for provider ${providerId}:`,
        error,
      );
      const errorMessage =
        error instanceof Error ? error.message : String(error);
  ...
  private setProviderRateLimitConfig(
    providerId: string,
    ...
    console.log(
      `[LLMProviderPresenter] Updated rate limit config for ${providerId}:`,
      newConfig,
    );
  ...
  private getOrCreateRateLimitState(
    providerId: string,
  ...
  private getCurrentQps(providerId: string): number {
    const state = this.providerRateLimitStates.get(providerId);
    if (!state || !state.config.enabled || state.lastRequestTime === 0)
      return 0;
    const now = Date.now();
  ...
  private cleanupProviderRateLimit(providerId: string): void {
    ...
    if (state) {
      ...
      console.log(
        `[LLMProviderPresenter] Cleaned up rate limit state for ${providerId}`,
      );
    }
  }
