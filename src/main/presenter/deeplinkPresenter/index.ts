...
export class DeeplinkPresenter implements IDeeplinkPresenter {
      ...
      app.on('second-instance', (_event, commandLine) => {
        ...
        if (process.platform === 'win32') {
          ...
          if (deepLinkUrl) {
            if (!app.isReady()) {
              console.log(
                'Windows: App not ready yet, saving URL:',
                deepLinkUrl,
              );
  ...
  async handleStart(params: URLSearchParams): Promise<void> {
    console.log(
      'Processing start command, parameters:',
  ...
  private async ensureChatTabActive(windowId: number): Promise<void> {
    try {
      ...
      const chatTab = tabsData.find(
        (tab) =>
          tab.url === 'local://chat' ||
          tab.url.includes('#/chat') ||
          tab.url.endsWith('/chat'),
      );
      if (chatTab) {
        if (!chatTab.isActive) {
        ...
        const newTabId = await tabPresenter.createTab(
          windowId,
          'local://chat',
          { active: true },
        );
        if (newTabId) {
          console.log(
            `[Deeplink] Waiting for tab ${newTabId} renderer to be ready`,
          );
    ...
    return new Promise((resolve) => {
      ...
      const timeoutId = setTimeout(() => {
        if (!resolved) {
          ...
          eventBus.off('tab:renderer-ready', onTabReady);
          console.log(
            `[Deeplink] Timeout waiting for tab ${tabId}, proceeding anyway`,
          );
          resolve();
        }
      }, 3000);
  ...
  async handleMcpInstall(params: URLSearchParams): Promise<void> {
    ...
    try {
      ...
      for (const [serverName, serverConfig] of Object.entries(
        ...
        if (serverConfig.type) {
          if (serverConfig.type === 'stdio' || serverConfig.type === 'sse') {
            ...
            if (determinedType === 'sse' && !determinedUrl) {
              console.error(
                `Server ${serverName} is type 'sse' but missing required 'url' field`,
              );
              continue;
            }
          } else {
          ...
          const hasCommand =
            !!determinedCommand && determinedCommand.trim() !== '';
        ...
        if (!determinedType) {
          console.error(
            `Cannot determine server ${serverName} type ('stdio' or 'sse')`,
          );
          continue;
        }
        ...
        const finalConfig: MCPServerConfig = {
          ...
          descriptions:
            serverConfig.descriptions || defaultConfig.descriptions!,
          ...
          baseUrl:
            determinedType === 'sse' ? determinedUrl! : defaultConfig.baseUrl!,
        };
