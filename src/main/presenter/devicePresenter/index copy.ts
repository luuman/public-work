...
// 将 exec 转换为 Promise 版本，方便 async/await 使用
const execAsync = promisify(exec)

/**
 * DevicePresenter 设备信息与操作实现类
 * 实现 IDevicePresenter 接口，用于获取系统信息、缓存图片、清理数据等功能
 */
export class DevicePresenter implements IDevicePresenter {
  ...
  /**
   * 获取磁盘使用情况
   * Windows 用 wmic 命令，Mac/Linux 用 df 命令
   */
  ...
  /**
   * 按类型重置数据
   * @param resetType 'chat' | 'knowledge' | 'config' | 'all'
   */
