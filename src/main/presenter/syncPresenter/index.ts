...
export class SyncPresenter implements ISyncPresenter {
  ...
  private readonly APP_SETTINGS_PATH = path.join(
    app.getPath('userData'),
    'app-settings.json',
  );
  private readonly MCP_SETTINGS_PATH = path.join(
    app.getPath('userData'),
    'mcp-settings.json',
  );
  private readonly PROVIDER_MODELS_DIR_PATH = path.join(
    app.getPath('userData'),
    'provider_models',
  );
  ...
  private readonly MODEL_CONFIG_PATH = path.join(
    app.getPath('userData'),
    'model-config.json',
  );
  ...
  public async importFromSync(
    ...
    const appSettingsBackupPath = path.join(
      syncFolderPath,
      'app-settings.json',
    );
    const providerModelsBackupPath = path.join(
      syncFolderPath,
      'provider_models',
    );
    const modelConfigBackupPath = path.join(
      syncFolderPath,
      'model-config.json',
    );
    ...
    try {
      ...
      const tempAppSettingsPath = path.join(
        app.getPath('temp'),
        `app_settings_${Date.now()}.json`,
      );
      const tempProviderModelsPath = path.join(
        app.getPath('temp'),
        `provider_models_${Date.now()}`,
      );
      const tempMcpSettingsPath = path.join(
        app.getPath('temp'),
        `mcp_settings_${Date.now()}.json`,
      );
      const tempModelConfigPath = path.join(
        app.getPath('temp'),
      ...
      if (fs.existsSync(this.PROVIDER_MODELS_DIR_PATH)) {
        this.copyDirectory(
          this.PROVIDER_MODELS_DIR_PATH,
          tempProviderModelsPath,
        );
      }
  ...
  private async performBackup(): Promise<void> {
    ...
    try {
      ...
      const tempDbBackupPath = path.join(
        syncFolderPath,
        `chat_${Date.now()}.db.tmp`,
      );
      ...
      const finalAppSettingsBackupPath = path.join(
        syncFolderPath,
        'app-settings.json',
      );
      const finalProviderModelsBackupPath = path.join(
        syncFolderPath,
        'provider_models',
      );
      const finalMcpSettingsBackupPath = path.join(
        syncFolderPath,
        'mcp-settings.json',
      );
      const finalModelConfigBackupPath = path.join(
        syncFolderPath,
        'model-config.json',
      );
      ...
      // 备份配置文件（过滤掉同步相关的设置）
      if (fs.existsSync(this.APP_SETTINGS_PATH)) {
        const appSettingsContent = fs.readFileSync(
          this.APP_SETTINGS_PATH,
          'utf-8',
        );
