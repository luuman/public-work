...
export class KnowledgeStorePresenter {
  ...
  // 文件处理进度跟踪器
  private fileProgressMap = new Map<
    string,
    { completed: number; error: number; total: number }
  >();
  ...
  private async processFileAsync(
    ...
    try {
      // 1. 读取文件和获取基本信息
      const mimeType = await presenter.filePresenter.getMimeType(
        fileMessage.path,
      );
      ...
      // 7. 初始化文件进度跟踪
      this.fileProgressMap.set(fileMessage.id, {
        completed: 0,
        error: 0,
        total: chunks.length,
      });
      ...
      await this.handleFileProcessingError(
        fileMessage.id,
  ...
  private async handleFileProcessingError(
    ...
    try {
      ...
      console.error(
        `[RAG] Error handling file processing error for ${fileId}:`,
        error,
      );
    }
  }
  ...
  async similarityQuery(key: string): Promise<QueryResult[]> {
    try {
      ...
      queryResults.forEach((res) => {
        res.distance = normalizeDistance(
          res.distance,
          getMetric(this.config.normalized),
        );
      });
      return queryResults;
    } catch (error) {
  ...
  async listFiles(): Promise<KnowledgeFileMessage[]> {
    try {
      ...
      console.error(
        `[RAG] Failed to list files in knowledge base ${this.config.id}:`,
        err,
      );
      throw err;
    }
  ...
  async resumeAllPausedTasks(): Promise<void> {
    ...
    const pausedChunkMessages = await this.vectorP.queryChunks({
      status: 'paused',
    });
    // count by file id
    ...
    for (const chunkMessage of pausedChunkMessages) {
      ...
      const chunkTask = {
        ...
        onSuccess: () =>
          this.handleChunkCompletion(chunkMessage.id, chunkMessage.fileId),
        onError: (error: Error) =>
        ...
        onTerminate: () =>
          console.log(
            `[RAG] Chunk processing terminated for ${chunkMessage.id}`,
          ),
      };
      this.taskP.addTask(chunkTask);
    }
  ...
  async destroy(): Promise<void> {
    try {
      ...
      console.error(
        `[RAG] Error destroying knowledge base ${this.config.id}:`,
        err,
      );
    }
  }
  ...
  async close(): Promise<void> {
    try {
      ...
      console.error(
        `[RAG] Error closing knowledge base ${this.config.id}:`,
        err,
      );
    }
  }
