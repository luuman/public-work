...
const {encryptStr, joinSep} = require('./encrypt.js');
 ...
 */
function patternLayout(pattern, tokens, password) {
    ...
    function safeStringify(item) {
        try {
            ...
            console.error('safeStringify log catch', e, item);
            return Object.prototype.toString.call(item);
        }
    }

    function formatMessage(loggingEvent) {
        const data = loggingEvent.data.map(item => {
            return Object.prototype.toString.call(item).slice(8, -1) === 'Error'
                ? util.format(item)
                : typeof item === 'object'
                ? safeStringify(item, '', 4)
                : item;
        });
    ...
    function fileName(loggingEvent, specifier) {
        ...
        filename = filename.replace('webpack:///./src/', '');
        filename = filename.replace('app://./js/', '');
...
const layoutMakers = {
    ...
    mypattern(config) {
        return patternLayout(config && config.pattern, config && config.tokens, config && config.password);
    }
};
