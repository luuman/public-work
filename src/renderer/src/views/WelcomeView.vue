...
const ModelIcon = defineAsyncComponent(
  () => import('@/components/icons/ModelIcon.vue'),
)
...
const providerModels = computed(() => {
  return (
    settingsStore.allProviderModels.find(
      (p) => p.providerId === selectedProvider.value,
    )?.models ?? []
  )
})
  ...
  if (currentStep.value < steps.length - 1) {
    if (currentStep.value === 1) {
      ...
      await settingsStore.updateProvider(selectedProvider.value, {
        ...
        id: settingsStore.providers.find(
          (p) => p.id === selectedProvider.value,
        )!.id,
        name: settingsStore.providers.find(
          (p) => p.id === selectedProvider.value,
        )!.name,
        apiType: settingsStore.providers.find(
          (p) => p.id === selectedProvider.value,
        )!.apiType,
        enable: true,
      })
...
const handleModelEnabledChange = async (
  ...
  try {
    await settingsStore.updateModelStatus(
      selectedProvider.value,
      model.id,
      !enabled,
    )
...
<template>
          ...
          <template v-else-if="currentStep === 1">
            <div class="space-y-6 max-w-xs mx-auto">
              ...
              <div class="mt-6 space-y-4">
                ...
                <div
                  v-show="selectedProvider !== 'ollama'"
                  class="flex flex-col gap-2"
                >
                  <Label for="api-key">{{
                    t('welcome.provider.apiKey')
                  }}</Label>
                  <Input
                    id="api-key"
                    v-model="apiKey"
  ...
  <Dialog v-model:open="showSuccessDialog">
    <DialogContent>
      <DialogHeader>
        <DialogTitle>{{
          t('settings.provider.dialog.verify.success')
        }}</DialogTitle>
        <DialogDescription>{{ dialogMessage }}</DialogDescription>
      </DialogHeader>
      <DialogFooter>
