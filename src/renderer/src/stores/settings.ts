...
import { CONFIG_EVENTS, OLLAMA_EVENTS, DEEPLINK_EVENTS } from '@/events/events'
...
export const useSettingsStore = defineStore('settings', () => {
  ...
  // 设置搜索助手模型
  const setSearchAssistantModel = async (
    model: RENDERER_MODEL_META,
    providerId: string,
  ) => {
  ...
  const initOrUpdateSearchAssistantModel = async () => {
    // 尝试从配置中加载搜索助手模型
    let savedModel = await configP.getSetting<{
      model: RENDERER_MODEL_META
      providerId: string
  ...
  const fontSizeClass = computed(
    () =>
      FONT_SIZE_CLASSES[fontSizeLevel.value] ||
  ...
  const loadProviderTimestamps = async () => {
    try {
      const savedTimestamps =
        await configP.getSetting<Record<string, number>>('providerTimestamps')
      if (savedTimestamps) {
  ...
  const initSettings = async () => {
    try {
      ...
      // 获取字体大小级别
      fontSizeLevel.value =
        (await configP.getSetting<number>('fontSizeLevel')) ??
        DEFAULT_FONT_SIZE_LEVEL
      // 确保级别在有效范围内
      if (
        fontSizeLevel.value < 0 ||
        fontSizeLevel.value >= FONT_SIZE_CLASSES.length
      ) {
      ...
      // 获取投屏保护设置
      contentProtectionEnabled.value =
        await configP.getContentProtectionEnabled()
      ...
      const activeEngineId =
        (await configP.getSetting<string>('searchEngine')) || 'google'
  ...
  const refreshCustomModels = async (providerId: string): Promise<void> => {
    try {
      ...
      // 更新自定义模型列表
      const customIndex = customModels.value.findIndex(
      ...
      // 更新已启用的模型列表
      const enabledIndex = enabledModels.value.findIndex(
        (item) => item.providerId === providerId,
      )
      if (enabledIndex !== -1) {
        ...
        const standardModels = currentModels.filter((model) => !model.isCustom)
        const enabledCustomModels = customModelsWithStatus.filter(
          (model) => model.enabled,
        )
        enabledModels.value[enabledIndex].models = [
          ...standardModels,
          ...enabledCustomModels,
        ]
      } else {
  ...
  const refreshStandardModels = async (providerId: string): Promise<void> => {
    try {
      ...
      const enabledModelsData = modelsWithStatus.filter(
        (model) => model.enabled,
      )
      if (enabledIndex !== -1) {
        // 保留自定义模型，更新标准模型
  ...
  const checkAndUpdateSearchAssistantModel = async (): Promise<void> => {
    if (searchAssistantModelRef.value) {
      ...
      const modelExists = provider?.models.some(
        (m) => m.id === searchAssistantModelRef.value?.id,
      )
  ...
  const refreshAllModels = useThrottleFn(
    _refreshAllModelsInternal,
    1000,
    true,
    true,
  )
  ...
  const updateFontSizeLevel = async (level: number) => {
    const validLevel = Math.max(
      0,
      Math.min(level, FONT_SIZE_CLASSES.length - 1),
    )
  ...
  const setupProviderListener = () => {
    ...
    window.electron.ipcRenderer.on(
      ...
      async (
        _event,
        msg: { providerId: string; modelId: string; enabled: boolean },
      ) => {
        // 只更新模型启用状态，而不是刷新所有模型
  ...
  const updateLocalModelStatus = (
    ...
    const providerIndex = allProviderModels.value.findIndex(
    ...
    const enabledProviderIndex = enabledModels.value.findIndex(
      (p) => p.providerId === providerId,
    )
    if (enabledProviderIndex !== -1) {
      ...
      if (enabled) {
        ...
        if (modelIndex === -1) {
          ...
          const provider = allProviderModels.value.find(
            (p) => p.providerId === providerId,
          )
    ...
    const customProviderIndex = customModels.value.findIndex(
      (p) => p.providerId === providerId,
    )
  ...
  const updateProviderConfig = async (
    ...
    const needRefreshProviders = ['name', 'enable'].some(
      (key) => key in updates,
    )
    ...
    // 只在特定条件下刷新模型列表
    const needRefreshModels = ['enable', 'apiKey', 'baseUrl'].some(
      (key) => key in updates,
    )
  ...
  const optimizeProviderOrder = async (
    providerId: string,
    enable: boolean,
  ): Promise<void> => {
    try {
  ...
  const removeProvider = async (providerId: string): Promise<void> => {
    try {
      ...
      // 从保存的顺序中移除此 provider
      providerOrder.value = providerOrder.value.filter(
        (id) => id !== providerId,
      )
  ...
  const enableAllModels = async (providerId: string): Promise<void> => {
    try {
      // 获取提供商的所有模型
      const providerModelsData = allProviderModels.value.find(
        (p) => p.providerId === providerId,
      )
      ...
      console.error(
        `Failed to enable all models for provider ${providerId}:`,
        error,
      )
      throw error
    }
  ...
  const disableAllModels = async (providerId: string): Promise<void> => {
    try {
      ...
      // 获取自定义模型
      const customModelsData = customModels.value.find(
        (p) => p.providerId === providerId,
      )
      ...
      console.error(
        `Failed to disable all models for provider ${providerId}:`,
        error,
      )
      throw error
    }
  ...
  const syncOllamaModelsToGlobal = async (): Promise<void> => {
    ...
    const enabledOllamaModels = ollamaModelsAsGlobal.filter(
      (model) => model.enabled,
    )
    ...
    window.electron.ipcRenderer.on(
      ...
      async () => {
        try {
          ...
          const currentActiveEngineId =
            await configP.getSetting<string>('searchEngine')
          if (currentActiveEngineId) {
            const engine = searchEngines.value.find(
              (e) => e.id === currentActiveEngineId,
            )
            if (engine) {
              activeSearchEngine.value = engine
