...
export function usePageCapture() {
  ...
  const getScrollTop = (
    ...
    if (isIframe && scrollContainer.tagName.toLowerCase() === 'iframe') {
      ...
      if (iframe.contentWindow) {
        return (
          iframe.contentWindow.scrollY || iframe.contentWindow.pageYOffset || 0
        )
      }
    }
  ...
  const captureArea = async (config: CaptureConfig): Promise<CaptureResult> => {
    ...
    try {
      ...
      // 记录容器原始滚动位置
      const containerOriginalScrollTop = getScrollTop(
        scrollContainer,
        isHTMLIframe,
      )
      ...
      // 计算可见截图窗口
      const captureWindowVisibleHeight =
        containerRect.height - containerHeaderOffset
      ...
      // 分段截图循环
      while (
        totalCapturedContentHeight < targetContentHeight &&
        ...
        if (!currentActualInitialRect) {
          console.error(
            `[CAPTURE_DEBUG] Iteration ${iteration}: 无法获取当前目标区域`,
          )
          break
        }
        ...
        const currentScroll = Math.max(
          0,
          Math.min(scrollTopTarget, maxScrollTop),
        )
        ...
        if (!finalInitialRectStateAfterScroll) {
          console.error(
            `[CAPTURE_DEBUG] Iteration ${iteration}: 滚动后无法获取目标区域`,
          )
          break
        }
        ...
        const contentOffsetYInCaptureWindow =
          topOfUncapturedContentInViewport - finalScRect.top
        ...
        if (contentOffsetYInCaptureWindow < 0) {
          ...
          heightToCaptureFromSegment = Math.min(
            targetContentHeight -
              totalCapturedContentHeight +
              contentOffsetYInCaptureWindow,
        ...
        // 执行截图
        try {
          const segmentData = await tabPresenter.captureTabArea(
            tabId,
            captureRect,
          )
          if (segmentData) {
            ...
            console.error(
              `[CAPTURE_DEBUG] Iteration ${iteration}: 截图失败，未返回数据`,
            )
            break
          }
        } catch (captureError) {
          console.error(
            `[CAPTURE_DEBUG] Iteration ${iteration}: 截图出错:`,
            captureError,
          )
          break
        }
      ...
      if (config.watermark) {
        finalImage = await tabPresenter.stitchImagesWithWatermark(
          imageDataList,
          config.watermark,
        )
      } else {
  ...
  const captureMessageRange = (
    ...
    getTargetRect: () => {
      const startElement = document.querySelector(
        `[data-message-id="${startMessageId}"]`,
      )
      const endElement = document.querySelector(
        `[data-message-id="${endMessageId}"]`,
      )
