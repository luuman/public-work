...
const type = ref<'sse' | 'stdio' | 'inmemory' | 'http'>(
  props.initialConfig?.type || 'stdio',
)
...
const isImageServer = computed(
  () => isInMemoryType.value && name.value === 'imageServer',
)
...
// 判断是否是powerpack服务器
const isPowerpackServer = computed(
  () => isInMemoryType.value && name.value === 'powerpack',
)
...
// 权限设置
const autoApproveAll = ref(
  props.initialConfig?.autoApprove?.includes('all') || false,
)
...
const showBaseUrl = computed(
  () => type.value === 'sse' || type.value === 'http',
)
// 添加计算属性来控制命令相关字段的显示
...
const showNpmRegistryInput = computed(() => {
  return (
    type.value === 'stdio' &&
    ['npx', 'node'].includes(command.value.toLowerCase())
  )
})
...
const isCommandValid = computed(() => {
  ...
  if (
    type.value === 'stdio' ||
    (isInMemoryType.value && !isImageServer.value)
  ) {
...
const isCustomHeadersFormatValid = computed(() =>
  validateKeyValueHeaders(customHeaders.value),
)
...
const handleSubmit = (): void => {
  ...
  try {
    ...
    toast({
      title: t('settings.mcp.serverForm.parseError'),
      description:
        t('settings.mcp.serverForm.customHeadersParseError') +
        ': ' +
        String(error),
      variant: 'destructive',
    })
  ...
  (newConfig) => {
    ...
    if (newConfig && props.editMode && !props.defaultJsonConfig) {
      ...
      if (props.serverName === 'powerpack' && newConfig.env) {
        ...
        useE2B.value =
          envConfig.USE_E2B === true || envConfig.USE_E2B === 'true'
      ...
      autoApproveRead.value =
        newConfig.autoApprove?.includes('read') ||
        newConfig.autoApprove?.includes('all') ||
        false
      autoApproveWrite.value =
        newConfig.autoApprove?.includes('write') ||
        newConfig.autoApprove?.includes('all') ||
        false
      ...
      <div class="space-y-2 px-4 pb-4">
        ...
        <div class="space-y-2">
          ...
          <Select v-model="type" :disabled="isFieldReadOnly">
            <SelectTrigger class="w-full">
              <SelectValue
                :placeholder="t('settings.mcp.serverForm.typePlaceholder')"
              />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="stdio">{{
                t('settings.mcp.serverForm.typeStdio')
              }}</SelectItem>
              <SelectItem value="sse">{{
                t('settings.mcp.serverForm.typeSse')
              }}</SelectItem>
              <SelectItem value="http">{{
                t('settings.mcp.serverForm.typeHttp')
              }}</SelectItem>
              <SelectItem
                v-if="
                  props.editMode && props.initialConfig?.type === 'inmemory'
                "
                value="inmemory"
        ...
        <div v-if="showFolderSelector" class="space-y-2">
          ...
          <div class="space-y-2">
            ...
            >
              <span class="text-sm truncate flex-1 mr-2" :title="folder">{{
                folder
              }}</span>
              <Button
                type="button"
            ...
            <input
              ...
              :placeholder="
                argumentsList.length === 0
                  ? t('settings.mcp.serverForm.argsPlaceholder')
                  : ''
              "
        ...
        <!-- 环境变量 -->
        <div
          v-if="(showCommandFields || isInMemoryType) && !isPowerpackServer"
          class="space-y-2"
        >
        ...
        >
          ...
          <div v-if="useE2B" class="space-y-2">
            ...
            <Input
              ...
              type="password"
              :placeholder="
                t('settings.mcp.serverForm.e2bApiKeyPlaceholder') ||
                '输入您的 E2B API Key'
              "
              required
            ...
            <div class="text-xs text-muted-foreground">
              {{
                t('settings.mcp.serverForm.e2bApiKeyHelp') ||
                '您可以在 E2B 控制台获取 API Key：'
              }}
              <a
            ...
            <div
              v-if="useE2B && !e2bApiKey.trim()"
              class="text-xs text-red-500"
            >
              {{
                t('settings.mcp.serverForm.e2bApiKeyRequired') ||
                'E2B API Key 是必需的'
              }}
            </div>
          </div>
        ...
        <div v-else class="space-y-2">
          <Label
            ...
            >{{ t('settings.mcp.serverForm.descriptions') }}</Label
          >
          <Input
        ...
        <div v-if="showBaseUrl" class="space-y-2">
          <Label
            class="text-xs text-muted-foreground"
            for="server-custom-headers"
            >{{ t('settings.mcp.serverForm.customHeaders') }}</Label
          >
          <Textarea
