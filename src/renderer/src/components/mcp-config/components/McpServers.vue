...
const getServerPromptsCount = (serverName: string) => {
  return mcpStore.prompts.filter((prompt) => prompt.client.name === serverName)
    .length;
};

// 计算属性：获取每个服务器的resources数量
const getServerResourcesCount = (serverName: string) => {
  return mcpStore.resources.filter(
    (resource) => resource.client.name === serverName,
  ).length;
};
...
<template>
  <div class="h-full flex flex-col">
      ...
      <div v-else class="space-y-4 py-3">
        ...
        <div v-if="inMemoryServers.length > 0">
          <div class="flex items-center space-x-2 mb-3">
            ...
            <div
              class="text-xs text-muted-foreground bg-muted/50 px-2 py-1 rounded-full"
            >
      ...
      <div class="flex items-center justify-between p-3">
        ...
        <div class="flex items-center space-x-3">
          <div class="flex items-center space-x-1">
            ...
            <span class="text-xs text-muted-foreground">
              ...
              {{ mcpStore.serverList.length }}
            </span>
          </div>
          <div
            v-if="mcpStore.serverList.length > 0"
        ...
        <div class="flex space-x-2">
            ...
            <DialogContent class="w-[90vw] max-w-[400px]">
              ...
              <DialogFooter class="flex-row gap-2 justify-end">
                ...
                <Button
                  variant="default"
                  size="sm"
                  @click="handleResetToDefaultServers"
                >
          ...
          <Dialog v-model:open="isAddServerDialogOpen">
            ...
            <DialogContent
              class="w-[95vw] max-w-[500px] px-0 h-[85vh] max-h-[500px] flex flex-col"
            >
              ...
              <McpServerForm
                :default-json-config="
                  settingsStore.mcpInstallCache || undefined
                "
    ...
    <!-- 工具调试弹窗 -->
    <McpToolPanel
      v-model:open="isToolPanelOpen"
      :server-name="selectedServerForTools"
    />

    <!-- 提示模板调试弹窗 -->
    <McpPromptPanel
      v-model:open="isPromptPanelOpen"
      :server-name="selectedServerForPrompts"
    />
