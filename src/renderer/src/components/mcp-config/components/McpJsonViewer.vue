...
<template>
  <div class="w-full">
    ...
    >
      <h4 v-if="title" class="text-sm font-medium text-foreground">
        {{ title }}
      </h4>
    ...
    <!-- 内容区域 -->
    <div
      class="relative border border-border/50 rounded-lg overflow-hidden bg-muted/20"
    >
      ...
      <div v-else class="max-h-96 overflow-auto">
        ...
        <pre
          v-else
          class="text-content p-4 whitespace-pre-wrap break-words text-sm font-mono"
          >{{ content }}</pre
        >
      </div>
    </div>
