<template>
  ...
  <div>
    ...
    >
      ...
      <div
        class="flex-shrink-0 px-2 rounded-lg rounded-l-none flex justify-center items-center"
      >
        <Icon
...
const isBlockError = () => {
  ...
  if (
    props.blockStatus !== 'loading' &&
    props.block.tool_call?.status === 'calling'
  ) {
    return true;
  }
  if (
    props.blockStatus !== 'loading' &&
    props.block.tool_call?.status === 'response'
  ) {
    return true;
  }
  return false;
