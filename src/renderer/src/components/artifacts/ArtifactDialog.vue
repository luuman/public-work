    ...
    >
      ...
      >
        <div class="flex items-center gap-2 flex-grow w-0">
          <button
            class="p-2 hover:bg-accent/50 rounded-md"
            @click="artifactStore.hideArtifact"
          >
          ...
          <div class="flex items-center gap-1">
            <Button
              v-if="
                ...
                artifactStore.currentArtifact?.type ===
...
const exportCode = () => {
  if (artifactStore.currentArtifact?.content) {
    ...
    const blob = new Blob([artifactStore.currentArtifact.content], {
      type: 'text/plain',
    })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
