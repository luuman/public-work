...
const t = (() => {
  try {
    ...
    return (key: string, values?: { name?: string; participants?: string }) => {
      if (key === 'artifacts.flowchartOf')
        return `流程图：${values?.name || ''}`;
      if (key === 'artifacts.sequenceDiagramBetween')
        return `时序图：${values?.participants || ''}`;
      if (key === 'artifacts.classDiagramOf')
        return `类图：${values?.name || ''}`;
      if (key === 'artifacts.stateDiagramOf')
        return `状态图：${values?.name || ''}`;
      ...
      if (key === 'artifacts.variableDefinition')
        return `变量：${values?.name || ''}`;
    ...
    if (chartTitle) {
      const finalTitle =
        chartType === 'flowchart'
          ...
          : chartType === 'sequence'
            ? t('artifacts.sequenceDiagramBetween', {
    ...
    case 'application/vnd.ant.code': {
      ...
      if (!foundTitle) {
        ...
        const functionMatch = content.match(
          /(?:function|def|func)\s+([a-zA-Z_]\w*)\s*\([^)]*\)/,
        );
        if (functionMatch) {
          ...
          if (classMatch) {
            ...
            if (content.includes('root.render(<App />);')) {
              ...
              if (varMatch) {
                codeTitle = t('artifacts.variableDefinition', {
                  name: varMatch[1],
                });
              }
            }
