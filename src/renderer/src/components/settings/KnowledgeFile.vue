    ...
    <Dialog v-model:open="isSearchDialogOpen">
      <TooltipProvider>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {{ t('settings.knowledgeBase.searchKnowledge') }}
            </DialogTitle>
          </DialogHeader>
          <div className="flex w-full items-center gap-1 relative">
            <Input
              v-model="searchKey"
              :placeholder="
                t('settings.knowledgeBase.searchKnowledgePlaceholder')
              "
            />
            <Button
              size="xs"
            ...
            <div class="relative min-h-[180px]">
              ...
              <div v-if="searchResult.length > 0">
                ...
                >
                  ...
                  <div
                    class="border-t border-gray-300 pt-2 mt-2 text-xs text-muted-foreground"
                  >
...
const ctrlBtn = computed(() => {
  if (fileList.value.length > 0) {
    const hasProcessing = fileList.value.find(
...
const loadList = async () => {
  fileList.value =
    (await knowledgePresenter.listFiles(props.builtinKnowledgeDetail.id)) || []
}

const toggleStatus = async (run: boolean) => {
  if (run) {
    await knowledgePresenter.resumeAllPausedTasks(
      props.builtinKnowledgeDetail.id,
    )
  } else {
    await knowledgePresenter.pauseAllRunningTasks(
      props.builtinKnowledgeDetail.id,
    )
  }
  loadList()
  ...
  if (e.dataTransfer?.files && e.dataTransfer.files.length > 0) {
    for (const file of e.dataTransfer.files) {
      ...
      try {
        ...
        const result = await knowledgePresenter.addFile(
          props.builtinKnowledgeDetail.id,
          path,
        )
        if (result.error) {
...
const reAddFile = async (file: KnowledgeFileMessage) => {
  const result = await knowledgePresenter.reAddFile(
    props.builtinKnowledgeDetail.id,
    file.id,
  )
