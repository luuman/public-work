  ...
  <div class="space-y-4 p-4 border rounded-lg bg-muted/30">
      ...
      <div v-if="status" class="space-y-2 text-xs">
        <div class="flex justify-between">
          <span class="text-muted-foreground"
            >{{ t('settings.rateLimit.lastRequestTime') }}:</span
          >
          <span class="font-mono">{{ formatLastRequestTime() }}</span>
        </div>
        <div class="flex justify-between">
          <span class="text-muted-foreground"
            >{{ t('settings.rateLimit.queueLength') }}:</span
          >
          <span class="font-mono">{{ status.queueLength }}</span>
        </div>
        <div class="flex justify-between">
          <span class="text-muted-foreground"
            >{{ t('settings.rateLimit.nextAllowedTime') }}:</span
          >
    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{{
            t('settings.rateLimit.confirmDisableTitle')
          }}</AlertDialogTitle>
...
const intervalValue = ref(
  convertQpsToInterval(props.provider.rateLimit?.qpsLimit ?? 0.1),
)
...
const updateRateLimitConfig = async () => {
  try {
    ...
    await llmPresenter.updateProviderRateLimit(
      props.provider.id,
...
const loadStatus = async () => {
  try {
    const rateLimitStatus = await llmPresenter.getProviderRateLimitStatus(
      props.provider.id,
    )
    status.value = {
...
const formatLastRequestTime = () => {
  ...
  if (diff < 60000)
    return `${Math.floor(diff / 1000)}${t('settings.rateLimit.secondsAgo')}`
...
const formatNextAllowedTime = () => {
  ...
  const nextAllowedTime =
    status.value.lastRequestTime + intervalValue.value * 1000
  const now = Date.now()
...
watch(
  ...
  (newProvider) => {
    ...
    intervalValue.value = convertQpsToInterval(
      newProvider.rateLimit?.qpsLimit ?? 0.1,
    )
    previousValidValue.value = intervalValue.value
    loadStatus()
  },
