  ...
  <div class="flex flex-col gap-4">
    ...
    >
      ...
      <div class="flex flex-row gap-2">
        ...
        >
          <div
            v-if="keyStatus.usage !== undefined"
            class="flex items-center gap-1"
          >
            <Icon icon="lucide:activity" class="w-3 h-3" />
            <span
              >{{ t('settings.provider.keyStatus.usage') }}:
              {{ keyStatus.usage }}</span
            >
          </div>
          <div
            v-if="keyStatus.limit_remaining !== undefined"
            class="flex items-center gap-1"
          >
...
const getKeyStatus = async () => {
  if (
    ...
    try {
      keyStatus.value = await llmProviderPresenter.getKeyStatus(
        props.provider.id,
      )
    } catch (error) {
