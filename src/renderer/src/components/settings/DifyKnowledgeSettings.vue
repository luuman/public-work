...
const saveDifyConfig = async () => {
  ...
  if (isEditing.value) {
    ...
    if (editingConfigIndex.value !== -1) {
      difyConfigs.value[editingConfigIndex.value] = {
        ...editingDifyConfig.value,
      }
    }
    toast({
