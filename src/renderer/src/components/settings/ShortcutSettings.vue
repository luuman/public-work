              ...
              <div class="relative w-full group">
                ...
                >
                  <span
                    ...
                    class="text-sm"
                    :class="{
                      'text-primary': !shortcutError,
                      'text-destructive': shortcutError,
                    }"
                  >
                ...
                <Button
                  v-if="
                    !shortcut.disabled && recordingShortcutId !== shortcut.id
                  "
                  variant="ghost"
                  size="sm"
...
const isShortcutConflict = (key: string, currentId: string): boolean => {
  // 检查快捷键是否已经被其他功能使用
  for (const [id, shortcut] of Object.entries<string>(
    shortcutKeys.value || {},
  )) {
...
const validateShortcut = (shortcut: string): boolean => {
  ...
  if (
    ...
    isShortcutConflict(shortcut, recordingShortcutId.value)
  ) {
...
const saveAndStopRecording = () => {
  if (shortcutKeys.value && recordingShortcutId.value && tempShortcut.value) {
    const shortcutKey =
      recordingShortcutId.value as keyof typeof shortcutKeys.value
