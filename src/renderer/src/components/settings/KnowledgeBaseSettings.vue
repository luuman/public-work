<template>
  <ScrollArea class="w-full h-full p-2">
    <div
      v-show="!showBuiltinKnowledgeDetail"
      class="w-full h-full flex flex-col gap-1.5"
    >
      <!-- 知识库配置标题 -->
      ...
      <div class="space-y-4 px-2 pb-4">
        ...
        >
          <span class="text-sm">{{
            t('settings.knowledgeBase.moreComingSoon')
          }}</span>
        </div>
      </div>
...
const difySettingsRef = ref<InstanceType<typeof DifyKnowledgeSettings> | null>(
  null,
);
const ragflowSettingsRef = ref<InstanceType<
  typeof RagflowKnowledgeSettings
> | null>(null);
const fastGptSettingsRef = ref<InstanceType<
  typeof FastGptKnowledgeSettings
> | null>(null);
const builtinSettingsRef = ref<InstanceType<
  typeof BuiltinKnowledgeSettings
> | null>(null);
