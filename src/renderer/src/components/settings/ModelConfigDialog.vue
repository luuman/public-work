<template>
  <Dialog :open="open" @update:open="$emit('update:open', $event)">
    ...
    >
      <DialogHeader>
        <DialogTitle
          >{{ t('settings.model.modelConfig.title') }} -
          {{ modelName }}</DialogTitle
        >
      ...
      <div class="overflow-y-auto flex-1 pr-2 -mr-2">
        <form @submit.prevent="handleSave" class="space-y-6">
          ...
          <div class="space-y-2">
            <Label for="maxTokens">{{
              t('settings.model.modelConfig.maxTokens.label')
            }}</Label>
            <Input
              id="maxTokens"
          ...
          <div class="space-y-2">
            <Label for="temperature">{{
              t('settings.model.modelConfig.temperature.label')
            }}</Label>
            <Input
              id="temperature"
          ...
          <div class="space-y-2">
            <Label for="type">{{
              t('settings.model.modelConfig.type.label')
            }}</Label>
            <Select v-model="config.type">
              <SelectTrigger>
                <SelectValue
                  :placeholder="t('settings.model.modelConfig.type.label')"
                />
              </SelectTrigger>
              <SelectContent>
          ...
          <div class="flex items-center justify-between">
            <div class="space-y-0.5">
              <Label>{{
                t('settings.model.modelConfig.functionCall.label')
              }}</Label>
              <p class="text-xs text-muted-foreground">
          ...
          <div class="flex items-center justify-between">
            <div class="space-y-0.5">
              <Label>{{
                t('settings.model.modelConfig.reasoning.label')
              }}</Label>
              <p class="text-xs text-muted-foreground">
            ...
            <div class="space-y-3 pl-4 border-l-2 border-muted">
              ...
              <div class="space-y-2">
                ...
                <p class="text-xs text-muted-foreground">
                  ...
                  <span
                    v-else-if="props.modelId.includes('pro')"
                    class="text-red-600 font-medium"
                  >
                    {{ t('settings.model.modelConfig.thinkingBudget.notice')
                    }}{{
                      t(
                        'settings.model.modelConfig.thinkingBudget.warnings.proNoDisable',
                      )
                    }}。
                  </span>
                  {{
                    t(
                      'settings.model.modelConfig.thinkingBudget.dynamicPrefix',
                    )
                  }}{{ getDisableHint() }}，{{
                    t(
                      'settings.model.modelConfig.thinkingBudget.range',
                      thinkingBudgetRange,
                    )
                  }}
                </p>
  ...
  <Dialog :open="showResetConfirm" @update:open="showResetConfirm = $event">
    <DialogContent class="sm:max-w-[425px]">
      <DialogHeader>
        <DialogTitle>{{
          t('settings.model.modelConfig.resetConfirm.title')
        }}</DialogTitle>
        <p class="text-sm text-muted-foreground">
...
const validateForm = () => {
  ...
  if (!config.value.maxTokens || config.value.maxTokens <= 0) {
    errors.value.maxTokens = t(
      'settings.model.modelConfig.validation.maxTokensMin',
    )
  } else if (config.value.maxTokens > 1000000) {
    errors.value.maxTokens = t(
      'settings.model.modelConfig.validation.maxTokensMax',
    )
  }

  // 验证上下文长度
  if (!config.value.contextLength || config.value.contextLength <= 0) {
    errors.value.contextLength = t(
      'settings.model.modelConfig.validation.contextLengthMin',
    )
  } else if (config.value.contextLength > 100_000_000) {
    errors.value.contextLength = t(
      'settings.model.modelConfig.validation.contextLengthMax',
    )
  }

  // 验证温度
  if (config.value.temperature < 0) {
    errors.value.temperature = t(
      'settings.model.modelConfig.validation.temperatureMin',
    )
  } else if (config.value.temperature > 2) {
    errors.value.temperature = t(
      'settings.model.modelConfig.validation.temperatureMax',
    )
  }
}
...
const handleSave = async () => {
  ...
  try {
    await settingsStore.setModelConfig(
      props.modelId,
      props.providerId,
      config.value,
    )
...
const thinkingBudgetRange = computed(() => {
  const modelConfig = getThinkingBudgetConfig(props.modelId)
  return (
    modelConfig || { min: 128, max: 32768, defaultValue: -1, canDisable: false }
  )
})
...
const thinkingBudgetError = computed(() => {
  ...
  if (value === 0 && !range.canDisable) {
    if (props.modelId.includes('pro')) {
      return t(
        'settings.model.modelConfig.thinkingBudget.warnings.proCannotDisable',
      )
    } else if (props.modelId.includes('flash-lite')) {
      return t(
        'settings.model.modelConfig.thinkingBudget.warnings.flashLiteCannotSetZero',
      )
    } else {
      return t(
        'settings.model.modelConfig.thinkingBudget.warnings.modelCannotDisable',
      )
    }
  }
  ...
  if (value < range.min && value !== 0) {
    ...
    if (props.modelId.includes('flash-lite') && value > 0 && value < 512) {
      return t(
        'settings.model.modelConfig.thinkingBudget.warnings.flashLiteMinValue',
      )
    }
    ...
    if (range.canDisable && range.min === 0) {
      hint = t(
        'settings.model.modelConfig.thinkingBudget.hints.withZeroAndDynamic',
      )
  ...
  if (value > range.max) {
    return t('settings.model.modelConfig.thinkingBudget.warnings.aboveMax', {
      max: range.max,
    })
  }
  return ''
