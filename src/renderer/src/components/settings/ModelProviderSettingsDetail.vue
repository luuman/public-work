...
const _initData = async () => {
  ...
  const customModelData = settingsStore.customModels.find(
    (p) => p.providerId === props.provider.id,
  )
  if (customModelData) {
  ...
  if (props.provider.id === 'gemini') {
    ...
    for (const key in safetyCategories) {
      ...
      try {
        const savedValue = (await settingsStore.getGeminiSafety(
          categoryKey,
        )) as string | 'HARM_BLOCK_THRESHOLD_UNSPECIFIED'
        ...
        console.log(
          `Set Gemini level for ${categoryKey}:`,
          geminiSafetyLevels[categoryKey],
        )
      } catch (error) {
        console.error(
          `Failed to fetch Gemini safety setting for ${categoryKey}:`,
          error,
        )
        geminiSafetyLevels[categoryKey] =
    ...
    console.log(
      'All Gemini safety levels initialized:',
      JSON.stringify(geminiSafetyLevels),
    )
  }
}
...
const handleSafetySettingChange = async (
  ...
  if (value) {
    ...
    console.log(
      `Gemini safety setting for ${key} updated to level ${level} (${value})`,
    )
  }
}
