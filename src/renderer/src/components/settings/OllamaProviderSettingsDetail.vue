  ...
  <section class="w-full h-full">
        ...
        <div class="flex flex-col w-full gap-2 mt-2">
          ...
          <div class="flex flex-col w-full border overflow-hidden rounded-lg">
            ...
            >
              ...
              <template v-else>
                <div
                  class="flex flex-row items-center justify-between p-2 hover:bg-accent"
                >
                  <div class="flex flex-col flex-grow">
                    <div class="flex flex-row items-center gap-1">
                      ...
                      <span class="w-[50px]">
                        <Progress
                          :model-value="pullingModels.get(model.name)"
                          class="h-1.5"
                        />
                      </span>
                    </div>
    ...
    <Dialog v-model:open="showPullModelDialog">
      <DialogContent class="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{{
            t('settings.provider.dialog.pullModel.title')
          }}</DialogTitle>
        </DialogHeader>
    ...
    <Dialog v-model:open="showDeleteModelDialog">
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{{
            t('settings.provider.dialog.deleteModel.title')
          }}</DialogTitle>
        </DialogHeader>
        <div class="py-4">
          {{
            t('settings.provider.dialog.deleteModel.content', {
              name: modelToDelete,
            })
          }}
        </div>
...
const availableModels = computed(() => {
  ...
  return presetModels.filter(
    (m) => !localModelNames.has(m.name) && !pullingModelNames.has(m.name),
  )
})
...
const showDeleteModelConfirm = (modelName: string) => {
  if (isModelRunning(modelName)) {
    toast({
      ...
      description: t('settings.provider.toast.modelRunningDesc', {
        model: modelName,
      }),
      variant: 'destructive',
      duration: 3000,
