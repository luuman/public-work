<template>
  <Dialog v-model:open="isOpen" @update:open="onOpenChange">
    ...
    >
      <DialogHeader>
        <DialogTitle>{{
          t('settings.provider.dialog.modelCheck.title')
        }}</DialogTitle>
        <DialogDescription>
          ...
          <div class="flex items-start">
            ...
            <div class="text-red-800 min-w-0 flex-1">
              <div class="font-medium">
                {{ t('settings.provider.dialog.modelCheck.failed') }}
              </div>
              <div
                class="text-sm mt-1 break-words whitespace-pre-wrap overflow-y-auto max-h-40"
              >
                {{ result.errorMsg }}
              </div>
            </div>
...
const handleCheck = async () => {
  ...
  try {
    ...
    // 调用设置store的检查方法
    const checkResult = await settingsStore.checkProvider(
      props.providerId,
      selectedModelId.value,
    )
