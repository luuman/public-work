  ...
  <div class="flex flex-col items-start gap-2">
    ...
    <div v-if="validationResult" class="w-full">
      ...
      >
        <Icon
          :icon="
            validationResult.success ? 'lucide:check-circle' : 'lucide:x-circle'
          "
          :class="[
            'w-4 h-4',
            validationResult.success
...
const validationResult = ref<{ success: boolean; message: string } | null>(
  null,
);
...
const startDeviceFlowLogin = async () => {
  ...
  try {
    const success = await oauthPresenter.startGitHubCopilotDeviceFlowLogin(
      props.provider.id,
    );
...
const startOAuthLogin = async () => {
  ...
  try {
    const success = await oauthPresenter.startGitHubCopilotLogin(
      props.provider.id,
    );
