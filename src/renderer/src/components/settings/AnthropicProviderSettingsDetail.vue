    ...
    <div class="w-full h-full p-2 flex flex-col gap-2 overflow-y-auto">
      ...
      <div class="flex flex-col items-start p-2 gap-2">
        <Label class="flex-1 cursor-pointer">{{
          t('settings.provider.authMethod')
        }}</Label>
        <Select
          v-model="authMethod"
          @update:model-value="
            (value: string) => switchAuthMethod(value as 'apikey' | 'oauth')
          "
        >
          <SelectTrigger class="w-full">
            <SelectValue
              :placeholder="t('settings.provider.authMethodPlaceholder')"
            />
          </SelectTrigger>
          <SelectContent>
      ...
      <div
        ...
        class="flex flex-col items-start p-2 gap-2"
      >
      ...
      <div
        v-if="
          ...
          (authMethod === 'oauth' && hasOAuthToken)
        "
      ...
      <Dialog v-model:open="showCodeDialog">
        ...
        >
          <DialogHeader>
            <DialogTitle>{{
              t('settings.provider.inputOAuthCode')
            }}</DialogTitle>
          </DialogHeader>
          <div class="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button
              :disabled="!oauthCode.trim() || isSubmittingCode"
...
const detectAuthMethod = async () => {
  ...
  try {
    ...
    if (props.provider.authMode) {
      ...
      const hasApiKey = !!(
        props.provider.apiKey && props.provider.apiKey.trim()
      )
      ...
      if (hasOAuth) {
        ...
        // 保存检测到的认证模式
        await settingsStore.updateProviderAuth(
          props.provider.id,
          'oauth',
          undefined,
        )
  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : t('settings.provider.loginFailed')
...
const submitOAuthCode = async () => {
  ...
  try {
    const success = await oauthPresenter.completeAnthropicOAuthWithCode(
    ...
    const message =
      error instanceof Error
        ? error.message
        : t('settings.provider.codeExchangeFailed')
  ...
  try {
    ...
    validationResult.value = {
      ...
      message:
        error instanceof Error
          ? error.message
          : t('settings.provider.disconnectFailed'),
    }
  }
}
  ...
  try {
    ...
    validationResult.value = {
      ...
      message:
        error instanceof Error
          ? error.message
          : t('settings.provider.oauthVerifyFailed'),
    }
  }
}
