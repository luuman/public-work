<template>
  <ScrollArea class="w-full h-full p-2">
    <div class="w-full h-full flex flex-col gap-1.5">
      ...
      <div class="flex flex-row p-2 items-center gap-2 px-2">
        ...
        >
          ...
          <span class="text-sm font-medium">{{
            t('settings.common.searchEngine')
          }}</span>
        </span>
        <div class="flex-shrink-0 flex gap-2">
          <div class="min-w-52 max-w-96">
            <Select v-model="selectedSearchEngine" class="">
              <SelectTrigger>
                <SelectValue
                  :placeholder="t('settings.common.searchEngineSelect')"
                />
              </SelectTrigger>
              <SelectContent>
                <SelectItem
          ...
          <Button
            ...
            @click="
              currentEngine && openDeleteSearchEngineDialog(currentEngine)
            "
          >
            ...
            <PopoverTrigger as-child>
              <Button variant="outline" class="w-full justify-between">
                <div class="flex items-center gap-2">
                  ...
                  />
                  <span class="truncate">{{
                    selectedSearchModel?.name ||
                    t('settings.common.selectModel')
                  }}</span>
                </div>
      ...
      <div class="flex flex-row p-2 items-center gap-2 px-2">
        ...
        <div class="flex-shrink-0 min-w-64 max-w-96">
          <Select v-model="selectedProxyMode" class="">
            <SelectTrigger>
              <SelectValue
                :placeholder="t('settings.common.proxyModeSelect')"
              />
            </SelectTrigger>
            <SelectContent>
              <SelectItem
      ...
      <div
        v-if="selectedProxyMode === 'custom'"
        class="flex flex-col p-2 gap-2 px-2"
      >
        <div class="flex flex-row items-center gap-2">
          ...
          >
            ...
            <span class="text-sm font-medium">{{
              t('settings.common.customProxyUrl')
            }}</span>
          </span>
          <div class="flex-shrink-0 min-w-64 max-w-96">
            <Input
      ...
      <div class="flex flex-row p-2 items-center gap-2 px-2">
        ...
        >
          ...
          <span class="text-sm font-medium">{{
            t('settings.common.searchPreview')
          }}</span>
        </span>
      ...
      <div class="flex flex-row p-2 items-center gap-2 px-2">
        ...
        >
          ...
          <span class="text-sm font-medium">{{
            t('settings.common.loggingEnabled')
          }}</span>
        </span>
        <div class="flex-shrink-0">
          <Switch
      ...
      <div class="flex flex-row p-2 items-center gap-2 px-2">
        ...
        >
          ...
          <span class="text-sm font-medium">{{
            t('settings.common.soundEnabled')
          }}</span>
        </span>
        <div class="flex-shrink-0">
          <Switch
            ...
            @update:checked="handleSoundChange"
          />
        </div>
      </div>
      ...
      <div class="flex flex-row p-2 items-center gap-2 px-2">
        ...
        >
          <Icon icon="lucide:file-text" class="w-4 h-4 text-muted-foreground" />
          <span class="text-sm font-medium">{{
            t('settings.common.copyWithCotEnabled')
          }}</span>
        </span>
        <div class="flex-shrink-0">
          <Switch
      ...
      <Dialog :open="isLoggingDialogOpen" @update:open="cancelLoggingChange">
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{{
              t('settings.common.loggingDialogTitle')
            }}</DialogTitle>
            <DialogDescription>
              <div class="space-y-2">
                <p>
          ...
          <DialogFooter>
            ...
            <Button @click="confirmLoggingChange">{{
              t('common.confirm')
            }}</Button>
          </DialogFooter>
        </DialogContent>
      ...
      >
        ...
        />
        <span class="text-sm font-medium">{{
          t('settings.common.openLogFolder')
        }}</span>
      </div>
      <!-- 重置数据 -->
  ...
  <Dialog v-model:open="isAddSearchEngineDialogOpen">
    <DialogContent>
      <DialogHeader>
        <DialogTitle>{{
          t('settings.common.addCustomSearchEngine')
        }}</DialogTitle>
        <DialogDescription>
      ...
      <DialogFooter>
        ...
        <Button
          type="submit"
          :disabled="!isValidNewSearchEngine"
          @click="addCustomSearchEngine"
        >
          {{ t('dialog.confirm') }}
        </Button>
      </DialogFooter>
  ...
  <Dialog v-model:open="isDeleteSearchEngineDialogOpen">
    <DialogContent>
      <DialogHeader>
        <DialogTitle>{{
          t('settings.common.deleteCustomSearchEngine')
        }}</DialogTitle>
        <DialogDescription>
          {{
            t('settings.common.deleteCustomSearchEngineDesc', {
  ...
  <Dialog v-model:open="isTestSearchEngineDialogOpen">
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          {{
            t('settings.common.testSearchEngineDesc', {
...
const selectedSearchEngine = ref(
  settingsStore.activeSearchEngine?.id ?? 'google',
)
...
const addCustomSearchEngine = async () => {
  ...
  try {
    ...
    settingsStore.searchEngines.splice(
      0,
      settingsStore.searchEngines.length,
      ...allEngines,
    )
...
const deleteCustomSearchEngine = async () => {
  ...
  try {
    ...
    // 记录被删除的是否为当前选中的引擎
    const isDeletingActiveEngine =
      selectedSearchEngine.value === engineToDelete.value?.id

    // 过滤掉要删除的搜索引擎
    customSearchEngines = customSearchEngines.filter(
      (e) => e.id !== engineToDelete.value?.id,
    )
    ...
    if (isDeletingActiveEngine) {
      // 找到第一个默认引擎 (非自定义引擎)
      const firstDefaultEngine = settingsStore.searchEngines.find(
        (e) => !e.isCustom,
      )
...
const currentEngine = computed(() => {
  return (
    settingsStore.searchEngines.find(
      (engine) => engine.id === selectedSearchEngine.value,
    ) || null
  )
})
