    ...
    <Dialog v-model:open="isFastGptConfigDialogOpen">
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button
            type="button"
            :disabled="!isEditingFastGptConfigValid"
...
const saveFastGptConfig = async () => {
  ...
  if (isEditing.value) {
    ...
    if (editingConfigIndex.value !== -1) {
      fastGptConfigs.value[editingConfigIndex.value] = {
        ...editingFastGptConfig.value,
      }
    }
    toast({
