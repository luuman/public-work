              ...
              <span v-if="rateLimitStatus.queueLength > 0">
                {{
                  t('chat.input.rateLimitQueue', {
                    count: rateLimitStatus.queueLength,
                  })
                }}
              </span>
...
const getRateLimitStatusTooltip = () => {
  ...
  const waitTime = Math.ceil(
    (rateLimitStatus.value.lastRequestTime +
      intervalSeconds * 1000 -
      Date.now()) /
      1000,
  )
  return t('chat.input.rateLimitWaitingTooltip', {
    seconds: waitTime,
    interval: intervalSeconds,
  })
}
      ...
      if (block.type === 'paragraph') {
        ...
        for (const [index, subBlock] of (block.content ?? []).entries()) {
          if (subBlock.type === 'text') {
            ...
            try {
              if (
                subBlock.attrs?.category === 'resources' &&
                subBlock.attrs?.content
              ) {
                fetchingMcpEntry.value = true
                // console.log(subBlock.attrs?.content)
                const mcpEntry = JSON.parse(
                  subBlock.attrs?.content,
            ...
            if (subBlock.attrs?.category === 'prompts') {
              ...
              try {
                ...
                if (promptAttrContent) {
                  ...
                  content =
                    subBlock.attrs?.label || subBlock.attrs?.id || 'prompt'
                }
              } catch (error) {
...
const disabledSend = computed(() => {
  ...
  if (activeThreadId) {
    return (
      ...
      currentContextLength.value >
        (props.contextLength ?? chatStore.chatConfig.contextLength)
    )
  }
  return false
})
...
const handleEditorEnter = (e: KeyboardEvent) => {
  ...
  const hasMentionSuggestion =
    editor.isActive('mention') || document.querySelector('.tippy-box')
...
const initSettings = async () => {
  settings.value.deepThinking = Boolean(
    await configPresenter.getSetting('input_deepThinking'),
  )
  settings.value.webSearch = Boolean(
    await configPresenter.getSetting('input_webSearch'),
  )
    ...
    for (const file of e.dataTransfer.files) {
      try {
        ...
        if (file.type === '') {
          ...
          if (isDirectory) {
            const fileInfo: MessageFile =
              await filePresenter.prepareDirectory(path)
            if (fileInfo) {
              selectedFiles.value.push(fileInfo)
            }
          } else {
...
const loadRateLimitStatus = async () => {
  ...
  if (currentProviderId) {
    try {
      const status =
        await llmPresenter.getProviderRateLimitStatus(currentProviderId)
...
// 简化的插入mention到编辑器
const insertMentionToEditor = (
  mentionData: CategorizedData,
  position: number,
): boolean => {
  try {
    // 构建mention节点属性
...
const handlePostInsertActions = async (
  mentionData: CategorizedData,
): Promise<void> => {
  // 处理Prompt类型的特殊逻辑
  if (mentionData.category === 'prompts' && mentionData.mcpEntry) {
    ...
    if (
      promptEntry.files &&
      Array.isArray(promptEntry.files) &&
      promptEntry.files.length > 0
    ) {
