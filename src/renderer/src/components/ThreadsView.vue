    ...
    <Dialog v-model:open="renameDialog">
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button variant="default" @click="handleThreadRename">{{
            t('dialog.confirm')
          }}</Button>
        </DialogFooter>
      </DialogContent>
...
const dynamicScrollerRef = ref<InstanceType<typeof DynamicScroller> | null>(
  null,
)
...
const handleThreadRename = async () => {
  try {
    ...
    await chatStore.renameThread(
      renameThread.value.id,
      renameThread.value.title,
    )
  } catch (error) {
