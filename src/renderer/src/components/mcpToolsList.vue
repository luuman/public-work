...
const getEnabledToolCountByServer = (serverName: string) => {
  const serverTools = mcpStore.tools.filter(
    (tool) => tool.server.name === serverName,
  );
...
const getTotalEnabledToolCount = () => {
  if (chatStore.chatConfig.enabledMcpTools) {
    ...
    const filterList = mcpStore.tools.filter((item) =>
    ...
    <Popover>
      ...
      <PopoverContent class="w-80 p-0" align="start">
        ...
        <div class="p-2 border-b flex items-center justify-between">
          <div>
            <div class="text-sm font-medium" :dir="langStore.dir">
              {{ t('mcp.tools.enabled') }}
            </div>
            ...
            >
              <div class="p-2 hover:bg-accent flex items-center w-full">
                <div :dir="langStore.dir" class="w-full">
                  ...
                  >
                  <span v-else class="flex-grow truncate text-left text-sm">{{
                    server.name
                  }}</span>
                </div>
                <Popover>
                ...
                <Switch
                  ...
                  @click="onServerToggle(server.name)"
                >
                  <template #thumb>
