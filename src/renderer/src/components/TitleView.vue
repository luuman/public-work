    ...
    <div class="flex flex-row gap-2 items-center">
      ...
      <Popover v-model:open="modelSelectOpen">
        <PopoverTrigger as-child>
          <Button
            variant="outline"
            class="flex items-center gap-1.5 px-2 h-7 relative"
            size="sm"
          >
            <ModelIcon
            ...
            >
              <Icon
                :icon="getRateLimitIcon()"
                :class="getRateLimitIconClass()"
                class="w-3 h-3"
              />
            </div>
...
const getRateLimitTooltip = () => {
  ...
  return t('chat.rateLimit.waitingTooltip', {
    seconds: waitTime,
    interval: intervalSeconds,
  })
}
...
const loadRateLimitStatus = async () => {
  if (props.model?.providerId) {
    try {
      const status = await llmPresenter.getProviderRateLimitStatus(
