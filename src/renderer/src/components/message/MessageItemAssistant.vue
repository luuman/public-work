  ...
  >
    ...
    <div class="flex flex-col w-full space-y-1.5">
      ...
      <MessageToolbar
        ...
        :is-in-generating-thread="
          chatStore.generatingThreadIds.has(currentThreadId)
        "
...
const allVariants = computed(() => {
  ...
  const combinedVariants = messageVariants.map((variant) => {
    const cachedVariant = Array.from(
      chatStore.getGeneratingMessagesCache().values(),
    ).find((cached) => {
  ...
  if (action === 'retry') {
    ...
    switch (action) {
      case 'prev':
        ...
        break
      case 'next':
        currentVariantIndex.value < totalVariants.value - 1 &&
