<template>
  ...
  >
    ...
    <div class="flex flex-col w-full space-y-1.5 items-end">
      ...
      <!-- 消息内容 -->
      <div
        class="text-sm bg-[#EFF6FF] dark:bg-muted rounded-lg p-2 border flex flex-col gap-1.5"
      >
        <div
          v-show="message.content.files.length > 0"
        ...
        <div
          v-if="isEditMode"
          class="text-sm w-full min-w-[40vw] whitespace-pre-wrap break-all"
        >
          <textarea
            v-model="editedText"
...
const startEdit = () => {
  ...
  if (
    ...
    props.message.content.content.length > 0
  ) {
    const textBlocks = props.message.content.content.filter(
...
const saveEdit = async () => {
  ...
  try {
    ...
    if (newContent?.content && newContent.content.length > 0) {
      const nonTextBlocks = newContent.content.filter(
        (block) => block.type !== 'text',
      )
      newContent.content = [
    ...
    await threadPresenter.editMessage(
      props.message.id,
