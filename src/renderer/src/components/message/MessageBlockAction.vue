<template>
  ...
  >
    ...
    <div
      v-else-if="block.action_type === 'rate_limit'"
      ...
      <div class="flex flex-col gap-2 text-xs">
        <div class="flex justify-between">
          <span class="text-muted-foreground"
            >{{ t('chat.messages.rateLimitQueue') }}:</span
          >
          <span class="font-mono">{{ block.extra?.queueLength || 0 }}</span>
        </div>
        <div class="flex justify-between">
          <span class="text-muted-foreground"
            >{{ t('chat.messages.rateLimitEstimated') }}:</span
          >
          <span class="font-mono">{{
            formatEstimatedTime(block.extra?.estimatedWaitTime)
          }}</span>
        </div>
      </div>
    ...
    >
      <Icon icon="lucide:check" class="w-4 h-4" />{{
        t('components.messageBlockAction.continued')
      }}
    </div>
  </div>
</template>
...
const formatEstimatedTime = (
  estimatedWaitTime?: string | number | boolean | object[],
) => {
  if (
    !estimatedWaitTime ||
