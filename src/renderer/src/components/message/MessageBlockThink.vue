  ...
  >
    ...
    >
      ...
      <span class="flex-grow"
        ...
        <span>{{
          reasoningDuration > 0
            ? t('chat.features.thinkingDuration', [reasoningDuration])
            : ''
        }}</span>
      </span>
    </div>
...
const reasoningDuration = computed(() => {
  ...
  if (props.block.reasoning_time) {
    duration =
      (props.block.reasoning_time.end - props.block.reasoning_time.start) /
      1000;
  } else {
    duration =
      (props.usage.reasoning_end_time - props.usage.reasoning_start_time) /
      1000;
  }
  // 保留小数点后最多两位，去除尾随的0
