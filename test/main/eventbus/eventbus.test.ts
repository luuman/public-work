...
import { EventBus, SendTarget } from '../../../src/main/events/eventbus'
import type {
  IWindowPresenter,
  ITabPresenter,
...
describe('EventBus 事件总线', () => {
  ...
  describe('Tab相关功能', () => {
    ...
    it('应该能够发送事件到指定Tab', async () => {
      ...
      expect(mockTabView.webContents.send).toHaveBeenCalledWith(
        eventName,
        testData,
      )
    })
  ...
  describe('错误处理', () => {
    ...
    it('当Tab已销毁时应该显示警告', async () => {
      ...
      vi.mocked(mockTabPresenter.getTab).mockResolvedValue(
        destroyedTabView as any,
      )
      const consoleSpy = vi.spyOn(console, 'warn').mockImplementation(() => {})
